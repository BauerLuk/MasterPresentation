...
\date{28. August 2020, Graz} % \today für heutiges Datum verwenden
%\date{\today}
...
%				$\rightarrow$ Maxwellian velocity distributions
			\end{itemize}
...
%\begin{frame}
%\frametitle{Modeling of kinetic equilibria}
%%\vspace{-1.5cm}
...
%		\begin{center}
%			\vspace{-1.5cm}
%			\includegraphics[trim={5cm 1cm 0cm 0cm},clip,width=1\textwidth]{FIGURES/tetrahedron_guiding_center.eps}
%		\end{center}
%	\end{column}
...
%				\item integrate each line for one poloidal turn over 500 equidistant steps $\rightarrow$ save $(R,Z)$ together with poloidal and normalized toroidal flux $s$ and poloidal angle $\vartheta = 2\pi i/500$ with current step $i$
...
%	\item Analytical solution - Taylor series: 
%	\be{}
%	\textbf{z}=\textbf{z}_0 + \sum\limits_{k=1}^\infty \frac{\tau^k}{k!}\hat{\textbf{a}}^{k-1}\cdot (\textbf{b} + \hat{ \textbf{a}} \cdot\textbf{z}_0)
%	\ee
\end{itemize}
\end{frame}
